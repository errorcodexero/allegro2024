...
public class Start2Shoot4Action extends Action {
    ...
    public Start2Shoot4Action(AllegroRobot2024 robot, boolean mirror, double mvalue) throws Exception {
        ...
        p1_ = new SwerveHolonomicPathFollowerAction(robot.getSwerve(), "S2S4-P1", true, 0.2, mirror, mvalue);
        ...
        p5_ = new SwerveHolonomicPathFollowerAction(robot.getSwerve(), "S2S4-P5", true, 0.2, mirror, mvalue);
    ...
    private void path6State() {
        if (p6_.isDone() && start_collect_.isDone()) {
            robot_.getIntakeShooter().setAction(manual_shoot_, true) ;
            state_ = State.Shoot4 ;
        }
    }
