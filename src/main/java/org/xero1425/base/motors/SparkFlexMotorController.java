...
public class SparkFlexMotorController extends MotorController {
   public class SimState {
    ...
    public SparkFlexMotorController(String name, int canid, boolean leader) throws MotorRequestFailedException {
        super(name) ;
        ...
        if (RobotBase.isSimulation()) {
            ...
            try {
                ctrl_ = new CANSparkFlex(canid, MotorType.kBrushless);
            }
            catch(Exception ex) {
                System.out.println("Exception creating motor") ;
            }
    ...
    public void follow(IMotorController ctrl, boolean leader, boolean invert) throws BadMotorRequestException, MotorRequestFailedException {
        ...
        if (simstate_ == null) {
            if (!CANSparkFlex.class.isInstance(ctrl.getNativeController())) {
                throw new BadMotorRequestException(this, "cannot follow a motor that is of another type") ;
            }

            CANSparkFlex other = (CANSparkFlex)ctrl.getNativeController() ;
    ...
    public void set(XeroPidType type, double target) throws BadMotorRequestException, MotorRequestFailedException {
        ...
        if (simstate_ != null) {
            ...
            switch(type) {
                case Power:
                    checkError("could not set voltage", () -> pid_.setReference(target * 12.0, CANSparkFlex.ControlType.kVoltage)) ;
                    break ;            
                case Position:
                    checkError("could not set position", () -> pid_.setReference(target, CANSparkFlex.ControlType.kPosition)) ;
                    break ;            
                case Velocity:
