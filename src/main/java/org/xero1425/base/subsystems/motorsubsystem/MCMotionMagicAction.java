...
public class MCMotionMagicAction extends MotorAction {
    ...
    private static String [] columns_ = { 
        "time", 
        "target (%%posunits%%)", 
        "curpos-target (%%posunits%%)",
        "actual (%%posunits%%)",
        "velocity (%%velunits%%)",
        "curvel-target (%%velunits%%)",
        "voltage (v)",
        "error (percent)"
    } ;    
    ...
    public void run() throws Exception  {
        ...
        double mcpostarget = me.getEncoder().mapMotorToPhysical(me.getMotorController().getCurrentTargetPosition()) ;
        ...
        if (plot_id_ != -1) {
            ...
            data_[4] = vel ;
            data_[5] = mcveltarget ;
            data_[6] = me.getMotorController().getVoltage() ;
            data_[7] = error ;
    ...
    public String toString(int indent) {
        ...
        ret += " pos=" + Double.toString(me.getPosition()) ;
